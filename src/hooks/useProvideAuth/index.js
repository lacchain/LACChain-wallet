...
function useProvideAuth() {
  ...
  const encryptWithPassword = (data, _password) => encrypt(data, _password);

  const encryptWithMetamask = async (data) => {
    const key = encryptionKey || await getEncryptionKey();

    return ethUtil.bufferToHex(Buffer.from(JSON.stringify(
  ...
  async function signin(user, email, password) {
    setUser(user);
    if (!provider) await signinWithPassword(user, email, password);
    else await signinWithMetamask(user);
  }

  async function update(user) {
    setUser(user);
    if (provider) await signinWithMetamask(user);
    else await signinWithPassword(user, account, password);
  ...
  async function decrypt(data, _password) {
    ...
    if (_password) return await decryptWithPassword(data, _password);
