 ...
 */
export const checksum = (payload) => Buffer.from(keccak256(Buffer.from(payload))).subarray(0, 4);
...
export const tryDecodeDomain = (domain) => {
  try {
    ...
    if (!(specificTypeVersion === '0001' && specificType === '0001')) {
      const message = `unsupported domain specific type/domain specific type version: ${
        specificType
      }${specificTypeVersion}`;
      console.log(message);
      return {
