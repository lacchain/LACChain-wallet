...
export const resolveCredentialStatus = async (vc, proof) => {
  ...
  const verificationRegistryDetailsResponse = await getDetailsFromVerificationRegistry(
...
export const getDetailsFromVerificationRegistry = async (
  ...
  const verificationRegistryVersion = await verificationRegistryContractInstance.version();
  ...
  const isRevoked = !!details.isRevoked;
  return {
    error: false,
    message: undefined,
    data: {
...
const resolvePublicKeyFromVerificationMethod = async (
  ...
  try {
    ...
    const message = 'There was an error while trying to public key from didDocument';
    return {
      error: true,
      message,
      data: {},
...
const resolveOnchainTimeDetails = async (vc, proof) => {
  ...
  if (proof && proof.cryptosuite === 'ecdsa-jcs-2019') {
    ...
    if (ecdsaJcs2019CredentialHashResult.error) {
      const message = `Error computing ecdsa JCS2019 credential hash/digest: ${
...
const verifyEcdsaJcs2019ProofSignature = async (vc, proof, publicKey) => {
  ...
  const { hashData } = hashDataResponse.data;
  let proofBytes;
  try {
